...
- 方案要具体、可执行，避免空泛的建议`;
  ...
  async planNovel(userInput, onProgress) {
    ...
    const planSystemPrompt = this._enrichSystemPrompt(`你是一位资深小说策划编辑。请基于用户需求和市场研究，生成完整的小说创作方案。
...
${researchSummary || '（无搜索结果，请基于你的知识库进行创作）'}`, 'plan');
  ...
  async revisePlan(currentPlan, feedback, novelId, onProgress) {
    ...
    const systemPrompt = this._enrichSystemPrompt(`你是一位资深小说策划编辑。用户对当前的小说创作方案提出了修改意见，请根据反馈修订方案。
