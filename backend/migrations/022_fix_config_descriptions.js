...
exports.up = async function (knex) {
  const configs = [
    ...
    { config_key: 'chapters_per_batch', config_value: '1', description: '章节大纲每批生成章节数' },
    { config_key: 'allow_registration', config_value: 'true', description: '是否允许新用户注册（true/false）' },
    { config_key: 'captcha_enabled', config_value: 'false', description: '是否启用验证码（true/false，默认关闭）' },
  ...
  for (const cfg of configs) {
    ...
    if (existing) {
      await knex('site_config').where('config_key', cfg.config_key).update({
